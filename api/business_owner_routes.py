...
from schemas.business_owner_schemas import BusinessOwnerCreate, BusinessOwnerResponse
...
def update_business_owner(owner_id: int, owner_update: BusinessOwnerCreate, db: Session = Depends(get_db)):
    owner = db.query(BusinessOwner).filter(BusinessOwner.id == owner_id).first()
    if not owner:
        raise HTTPException(status_code=404, detail="Business owner not found")
